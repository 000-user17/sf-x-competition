...
def class_init(map_info):
    ...
    class sf:
        ...
        def __init__(self, map_json):
            ...
            self.agvs_paths_time = {}
            self.path_obstacles_time = {}
        ...
        def before_load(self, shelf_id):
            ...
            while agv_id<len(self.agvs) :
                ...
                for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                    ...
                    path = self.random_bfs(start, [end[0]+dx, end[1]+dy]) 
                ...
                if self.agvs_paths[agv_id] == []:
                    ...
                    if self.agvs[agv_id].occupy != -1:
                        ...
                        if is_remove and tuple(self.cargos[cargo_id].loc) in self.obstacles:
                            ...
                            #print("删除的障碍物坐标"+str(tuple(self.cargos[cargo_id].loc)))
                    ...
                    if tuple(agv_loc) in self.path_obstacles_time:
                        for pd in self.path_obstacles_time[tuple(agv_loc)]:
                            if pd == self.agvs_paths_time[agv_id]:
                                self.path_obstacles_time[tuple(agv_loc)].remove(pd)
                                break
                            
                    self.agvs_paths_time[agv_id] += 1
                    #print(agv_id)
                    if tuple(agv_loc) in self.obstacles:
                        
                        self.obstacles.remove(tuple(agv_loc)) #更新障碍物
                        #print("删除的障碍物坐标"+str(tuple(agv_loc)))
        ...
        def iscollided(self, path):
            
            iscollid = False
            end_path = path[-1]
            
            for i in range(len(path)):
                if path[i] in self.ends:
                    #如果路径上某一点是某一个终点位置，则必会冲突
                    iscollid = True
                    break
                
                
                if path[i] in self.path_obstacles_time:
                    path_time = self.path_obstacles_time[path[i]]
                    path_time.sort()
                    if i in path_time or (i+1) in path_time or (i-1) in path_time:
                    #如果某一个格子在路线上的某一时刻冲突了
                        iscollid = True
                        break
            
            if end_path in self.path_obstacles_time:
                #如果当前路径的终点在之前路径中出现过，则必冲突
                iscollid = True
                    
            if not iscollid:
                #如果没有碰撞，则把该路径放入path障碍中
                for i in range(len(path)):
                    if path[i] in self.path_obstacles_time:
                        self.path_obstacles_time[path[i]].append(i)
                    else:
                        self.path_obstacles_time[path[i]] = [i]
            return iscollid
        ...
        def onload(self):
            ...
            for agv_id in self.agvs:
                real_agv_id = agvs_ids_list[agv_id]
                ...
                for cargo_id in self.cargos:
                    ...
                    for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                        #print(self.path_obstacles)
                        if (end[0]+dx, end[1]+dy) in self.obstacles:
                            continue
                ...
                self.agvs_paths[real_agv_id] = final_path #保存对应AGV的运输线路[(1,1), (2,2),...]
                self.agvs[real_agv_id].occupy = cargo_id
                self.cargos[cargo_id].load = real_agv_id
                self.ends.append(final_path[-1])
                #print(final_path)
                #("障碍物"+str(self.obstacles))
        ...
        def delivery(self):
            ...
            for agv_id in self.agvs:
                ...
                if final_path == []: #没有可行路径，暂时停止
                    continue
                self.agvs_paths[agv_id] = final_path #保存对应AGV的运输线路
                self.ends.append(final_path[-1])
                #print(final_path)
                #print("AVG"+str(agv_id) + "运输" + str(cargo_id) +"货物到货架"+str(self.cargos[cargo_id].target))    
        ...
        def random_move(self):
            ...
            for i in self.agvs_paths:
                ...
                while (ranx, rany) in self.obstacles:
                    ranx = random.randint(0, self.map_height)
                    rany = random.randint(0, self.map_width)
                self.agvs_paths[i] = self.random_bfs(self.agvs[i].loc ,[ranx, rany])
